...
# Seleciona colunas relevantes para clusterização
features = ['Year', 'Month', 'Day', 'Hour', 'Latitude',
            'Collision Type Num', 'Injury Type Num',
            'Weekend Num', 'Primary Factor Num']
...
# Rodando K-Means
kmeans = KMeans(n_clusters=3, init='k-means++', max_iter=100, random_state=42, n_init=10)
labels = kmeans.fit_predict(X_scaled)

# Adiciona os rótulos dos clusters ao dataframe
df['Cluster'] = labels

# Plotando resultados (usando Latitude vs Hour como exemplo de 2D)
plt.scatter(df['Latitude'], df['Hour'], c=labels, cmap='viridis', s=50, alpha=0.6)

# Pegando centróides (já transformados pelo scaler → precisamos inverter para escala original)
centroids_scaled = kmeans.cluster_centers_
centroids = scaler.inverse_transform(centroids_scaled)

plt.scatter(centroids[:, features.index('Latitude')], 
            centroids[:, features.index('Hour')], 
            c='red', marker='*', s=200, label='Centroids')

plt.title('K-Means Clustering - Crashcar Dataset')
plt.xlabel('Latitude')
plt.ylabel('Hour')
plt.legend()

# Exportando para buffer (sem plt.show)
buffer = StringIO()
